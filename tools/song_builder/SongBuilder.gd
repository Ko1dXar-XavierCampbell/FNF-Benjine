...
func _ready():
	ResourceSaver._get_recognized_extensions(data)
#	update_builder()
...
func update_builder():
	...
	if data.song_folder_name.is_empty():
...
func _on_chart_info_changed():
	...
	var vals_as_text = PackedStringArray()
	var new_chart_info = {}
	...
	if len(vals_as_text) <= 0 || len(vals_as_text) % 2 != 0:
		...
		for i in range(0, len(vals_as_text), 2):
			var difficulty_idx = str_to_var(vals_as_text[i])
