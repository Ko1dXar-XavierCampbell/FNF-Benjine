...
func _ready():
	...
	for combo_num_idx in len(combo_nums):
		...
		combo_num_velocities.append(Vector2(randf_range(-5, 5), -flx_randi(140, 160)))
	...
	tween.connect("tween_completed", Callable(self, "on_tween_completed").bind(), CONNECT_DEFERRED)
	tween.start()
...
func on_tween_completed(obj, _key):
	if obj in combo_nums:
		for connection in tween.get_signal_connection_list("tween_completed"):
			connection.source.disconnect(connection.signal, Callable(connection.target, connection.method))
		queue_free()
