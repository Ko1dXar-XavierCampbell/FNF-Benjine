...
func deferred_resize(current_size, result):
	...
	if current_size != $TextBubble.size:
		resize_main()
...
func load_theme(filename):
	...
	# Box size
	call_deferred('deferred_resize', $TextBubble.size, theme.get_value('box', 'size', Vector2(910, 167)))
